...
def _parse_goal_tokens(s: str, mapping: str = "base_minute"):
    ...
    mapping:
      - "base_minute": 45+1 -> 45, 90+3 -> 90 (USATO per filtri e tutte le stats tranne la tabella timeband)
...
# =======================
# Filtri gol: PRIMO/SECONDO (usano base_minute)
# =======================
...
def calcola_first_to_score(df_to_analyze, period='ft'):
    ...
    for _, row in df_to_analyze.iterrows():
        ...
        if period == 'ft':
            gol_home = _parse_goal_tokens(gol_home_str, mapping="base_minute")
            gol_away = _parse_goal_tokens(gol_away_str, mapping="base_minute")
        elif period == 'ht':
            gh = _parse_goal_tokens(gol_home_str, mapping="base_minute")
            ga = _parse_goal_tokens(gol_away_str, mapping="base_minute")
            gol_home = [m for m in gh if m <= 45]
            gol_away = [m for m in ga if m <= 45]
        elif period == 'sh':
            gh = _parse_goal_tokens(gol_home_str, mapping="base_minute")
            ga = _parse_goal_tokens(gol_away_str, mapping="base_minute")
            gol_home = [m for m in gh if m > 45]
...
def mostra_distribuzione_timeband(df_to_analyze, min_start_display=0):
    ...
    cnt_45p_matches = 0
    cnt_45p_matches_2plus = 0
    cnt_90p_matches = 0
    cnt_90p_matches_2plus = 0
    ...
    for ((start_interval, end_interval), label) in bands:
        ...
        for _, row in df_to_analyze.iterrows():
            ...
            # contatori stoppage per questa partita
            ...
            for tok in tok_home:
                if '+' in tok:
                    ...
                    if base == 45:
                        ...
                        reg_home.append(base)  # altri recuperi esotici su base minuto
                else:
            ...
            # Aggiorna 45+ e 90+ per questa partita
            if min_start_display <= 45:
                tot_45p = h_45p + a_45p
                if tot_45p > 0:
                    cnt_45p_matches += 1
                if tot_45p >= 2:
                    cnt_45p_matches_2plus += 1
    ...
    if min_start_display <= 45:
        ...
        if idx_3145 is not None:
            risultati.insert(idx_3145 + 1, row_45p)
        else:
            risultati.append(row_45p)
