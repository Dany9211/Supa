...
st.title("Filtro Completo Matches (FT, HT, BTTS & Odd Minima)")
...
# Funzione per distribuzione FT/HT con odd minima
def mostra_distribuzione(df, col_risultato, titolo):
    ...
    odd_minime = [round(100/wr, 2) if wr > 0 else "-" for wr in winrate]
    ...
    st.subheader(f"WinRate & Odd Minima {titolo}")
    st.table(winrate_df)
...
# BTTS SI
if not filtered_df.empty:
    btts_si = filtered_df[(filtered_df["gol_home_ft"] > 0) & (filtered_df["gol_away_ft"] > 0)]
    count_btts = len(btts_si)
    total_games = len(filtered_df)
    perc_btts = round((count_btts / total_games) * 100, 2) if total_games > 0 else 0

    st.subheader("BTTS (Both Teams To Score)")
    st.write(f"**Partite BTTS SI:** {count_btts}")
    st.write(f"**Percentuale BTTS SI:** {perc_btts}%")
    if perc_btts > 0:
        st.write(f"**Odd Minima BTTS SI:** {round(100/perc_btts, 2)}")
